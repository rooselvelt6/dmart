...
mod proptests {
        ...
        (
            ...
            .prop_map(
                |(mrn, apellido, nombre, fecha_nac, sexo)| {
                    format!("PID|||{}^^^HOSP||{} ^{}||{}|{}", mrn, apellido, nombre, fecha_nac, sexo)
                },
            )
    }
    ...
    fn arb_obx_mnemonic() -> impl Strategy<Value = String> {
        prop_oneof![
            f32_range(40.0, 180.0).prop_map(move |val| format!("OBX|1|NM|HR^Heart Rate||{}|bpm||||||F|||20240821141030", val)),
            f32_range(5.0, 50.0).prop_map(move |val| format!("OBX|1|NM|RESP^Respiration Rate||{}|/min||||||F|||20240821141030", val)),
            f32_range(70.0, 100.0).prop_map(move |val| format!("OBX|1|NM|SPO2^SpO2||{}|%||||||F|||20240821141030", val)),
            f32_range(35.0, 42.0).prop_map(move |val| format!("OBX|1|NM|T1^Temperature||{}|Cel||||||F|||20240821141030", val)),
            f32_range(80.0, 200.0).prop_map(move |val| format!("OBX|1|NM|NIBPs^NIBP Systolic||{}|mmHg||||||F|||20240821141030", val)),
            f32_range(50.0, 130.0).prop_map(move |val| format!("OBX|1|NM|NIBPm^NIBP Mean||{}|mmHg||||||F|||20240821141030", val)),
            f32_range(40.0, 120.0).prop_map(move |val| format!("OBX|1|NM|NIBPd^NIBP Diastolic||{}|mmHg||||||F|||20240821141030", val)),
        ]
    }

    // Genera un mensaje ORU^R01 completo válido
    fn arb_oru_message() -> impl Strategy<Value = String> {
        (arb_msh(), arb_pid(), prop::collection::vec(prop_oneof![arb_obx_loinc(), arb_obx_mnemonic()], 1..=10))
    ...
    fn parse_valid_oru_returns_vitals() {
        ...
        runner.run(&strategy, |msg| {
            if let Ok(parsed) = parse_oru_message(&msg) {
                prop_assert!(!parsed.vitals.is_empty(), "Parsed message should have vitals");
                for v in &parsed.vitals {
                    prop_assert!(v.loinc.is_some(), "Vital should have LOINC code");
                    prop_assert!(!v.loinc.as_ref().unwrap().is_empty(), "LOINC should not be empty");
                    prop_assert!(v.value.is_finite(), "Vital value must be finite");
                }
                prop_assert!(!parsed.patient_ref.is_empty(), "Patient ref should not be empty");
                prop_assert!(parsed.timestamp.contains('T'), "Timestamp should be RFC3339");
            }
            Ok(())
        }).unwrap();
    }
    ...
    fn hl7_datetime_conversion_valid() {
        let mut runner = TestRunner::new(Config::with_cases(100));
        let strategy = (2000..2099u32, 1..=12u32, 1..=28u32, 0..=23u32, 0..=59u32, 0..=59u32);
        runner.run(&strategy, |(year, month, day, hour, min, sec)| {
            let dt = format!("{:04}{:02}{:02}{:02}{:02}{:02}", year, month, day, hour, min, sec);
