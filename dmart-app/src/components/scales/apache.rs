...
#[component]
pub fn ApacheIIScale(data: RwSignal<ApacheIIData>) -> impl IntoView {
    ...
    let severity_color = move || match severity.get() {
    ...
    // Paciente
    let gcs_total =
        Signal::derive(move || data.get().gcs_ojos + data.get().gcs_verbal + data.get().gcs_motor);
